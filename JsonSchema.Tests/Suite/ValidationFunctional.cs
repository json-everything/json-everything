...
public class ValidationFunctional
{
	...
	private static IEnumerable<TestCaseData> GetTests(string draftFolder)
	{
		...
		foreach (var fileName in fileNames)
		{
			...
			var collections = JsonSerializer.Deserialize<List<TestCollectionFunctional>>(contents, TestEnvironment.TestSuiteSerializationOptions);
	...
	public void Test(TestCollectionFunctional collection, TestCase test, string fileName, EvaluationOptions options)
	{
		Console.WriteLine();
		Console.WriteLine();
		...
		if (collection.IsOptional && result != test.Valid)
			Assert.Inconclusive("Test optional");
		Assert.AreEqual(test.Valid, result);
	}

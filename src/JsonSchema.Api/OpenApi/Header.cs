...
public class HeaderRef : Header, IComponentRef
{
	...
	async Task IComponentRef.Resolve(OpenApiDocument root, JsonSerializerOptions? options)
	{
		...
		IsResolved = await OpenApi.Ref.Resolve(root, Ref, copy, ApiSerializerContext.Default.Header, options);
	}
}

internal class HeaderJsonConverter : JsonConverter<Header>
{
	private const string _objectType = "header";
	...
	public override Header Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
	{
		...
		if (reference is not null)
			return new HeaderRef(reference)
			{
				Summary = summary,
	...
	public override void Write(Utf8JsonWriter writer, Header value, JsonSerializerOptions options)
	{
		...
		if (value is HeaderRef reference)
		{

...
public sealed class ExclusiveMinimumHandler : IKeywordHandler
{
    public void Build(
        JsonElement value,
    ...
    )
    {
        if (value.ValueKind != JsonValueKind.Number)
            throw new JsonSchemaException($"Invalid 'exclusiveMinimum' keyword value: expected number, found {value.ValueKind}");

        var exclusiveMinimum = value.GetDecimal();

        constraints["exclusiveMinimum"] = (instance, _) =>
        {

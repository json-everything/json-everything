...
public class PatchExtensionTests
{
	...
	public void CreatePatch()
	{
		...
		var patch = initial.CreatePatch(expected, TestSerializerContext.Default.Options);

		Assert.AreEqual(patchExpected, JsonSerializer.Serialize(patch, JsonPatchSerializerContext.Default.JsonPatch));
	}
	...
	private static readonly JsonSerializerOptions _ignoreWritingNullSerializerOptions = new(TestSerializerContext.Default.Options)
	{
	...
	public void Add()
	{
		...
		var expected = JsonSerializer.Deserialize(patchExpectedStr, JsonPatchSerializerContext.Default.JsonPatch)!;
	...
	public void Remove()
	{
		...
		var patchExpected = JsonSerializer.Deserialize<JsonPatch>(patchExpectedStr, JsonPatchSerializerContext.Default.JsonPatch)!;
	...
	public void ComplexObject()
	{
		...
		var patchBackExpected = JsonSerializer.Deserialize<JsonPatch>(patchBackExpectedStr, JsonPatchSerializerContext.Default.JsonPatch);

		var patch = initial.CreatePatch(expected, TestSerializerContext.Default.Options);
		var patchBack = expected.CreatePatch(initial, TestSerializerContext.Default.Options);
	...
	public void CreatePatch_ClearArray()
	{
		...
		var patch = model.CreatePatch(model2, TestSerializerContext.Default.Options);

		var final = patch.Apply(model, TestSerializerContext.Default.Options);
	...
	public void ApplyPatch_Respect_SerializationOptions()
	{
		...
		var patch = JsonSerializer.Deserialize<JsonPatch>(patchStr, JsonPatchSerializerContext.Default.JsonPatch)!;

		var options = new JsonSerializerOptions(TestSerializerContext.Default.Options) { PropertyNamingPolicy = JsonNamingPolicy.CamelCase };
	...
	public void CreatePatch_JsonContext()
	{
		...
		var patchJson = JsonSerializer.Serialize(patch, JsonPatchSerializerContext.Default.JsonPatch);

		Assert.AreEqual(patchExpected, patchJson);
	}

	private static readonly JsonSerializerOptions _indentedSerializerOptions = new(JsonPatchSerializerContext.Default.Options)
	{
		WriteIndented = true
	};

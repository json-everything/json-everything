...
public class ThenKeyword : IJsonSchemaKeyword, ISchemaContainer, IKeywordHandler
{
	public static ThenKeyword Handler { get; } = new(true);
	...
	bool IKeywordHandler.Evaluate(FunctionalEvaluationContext context)
	{
		...
		if (!context.Annotations.TryGetValue(IfKeyword.Name, out var annotation) ||
		    annotation.AsValue().GetBool() == false)
			return true;

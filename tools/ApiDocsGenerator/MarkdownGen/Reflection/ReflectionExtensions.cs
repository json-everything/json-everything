...
public static class ReflectionExtensions
{
	...
	/// </summary>
	private static readonly HashSet<Type> _genericTuples = new(new[]
	{
		typeof(ValueTuple<>),
		typeof(ValueTuple<,>),
		typeof(ValueTuple<,,>),
	...
	/// </summary>
	private static Dictionary<Type, string> KnownTypeNames => _knownTypeNames ??= CreateKnownTypeNamesDictionary();

	/// <summary>
	///     Create a dictionary of standard value types and a string type.
	/// </summary>
	/// <returns>Dictionary mapping types to type names</returns>
	private static Dictionary<Type, string> CreateKnownTypeNamesDictionary()
	{
	...
	///     True if typeNameConverter lambda function should be invoked for generic type name such as for the List name in case
	///     of List&lt;SomeType&gt;
	///     If the parameter value is false then typeNameConverter is not invoked for the generic type name and only the plain
	///     type name is returned.
	///     If the parameter value is true then typeNameConverter must handle generic type definitions carefully and avoid
	///     calling
	...
	///     if type names should be decorated in some way either by converting text to markdown or
	///     HTML links or adding some formatting.
	///     This method returns named tuples with field names like this (Type1 field1, Type2 field2).
	///     <paramref name="tupleFieldNames" /> parameter
	///     must be specified with all tuple field names stored in the same order as they are in compiler-generated
	///     TupleElementNames attribute.
	///     If you do not know what it is then the better and easier way is to use ToTypeNameString() methods that retrieve
	///     field names from attributes.
	/// </summary>
	/// <param name="type"></param>
	/// <param name="tupleFieldNames">
	///     The names of value tuple fields as stored in TupleElementNames attribute. This queue is
	...
	/// </param>
	/// <returns>Full type name</returns>
	private static string ToNameString(this Type type,
		...
		bool invokeTypeNameConverterForGenericType = false)
	{
		...
		else if (type.IsGenericType)
		{
			...
			if (_genericTuples.Contains(genericTypeDefinition))
			{
			...
			{
				var genericTypeName = invokeTypeNameConverterForGenericType ? typeNameConverter?.Invoke(genericTypeDefinition, tupleFieldNames) : null;
				newTypeName =

...
public static class DocXmlReaderExtensions
{
	...
	/// <returns></returns>
	public static IEnumerable<(PropertyInfo Info, CommonComments Comments)> Comments(
	...
	/// <returns></returns>
	public static IEnumerable<(MethodBase Info, MethodComments Comments)> Comments(
		this DocXmlReader reader,
		IEnumerable<MethodBase> methodInfos)
	{
		return methodInfos.Select(info => (info, Comments: reader.GetMethodComments(info, info.IsConstructor && info.GetParameters().Length == 0)!))
	...
	/// <returns></returns>
	public static IEnumerable<(FieldInfo Info, CommonComments Comments)> Comments(
		this DocXmlReader reader,

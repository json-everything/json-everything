...
public class OptionalDataTests
{
			...
			("foo", new JsonSchemaBuilder()
				...
				.Properties(
					("bar", new JsonSchemaBuilder()
						.Type(SchemaValueType.Integer)
						.OptionalData(("minimum", "2/minValue"))
					)
				)
			)
		);
	...
	private static JsonSchema ExternalRef { get; } = new JsonSchemaBuilder()
		...
		.Properties(
			("foo", new JsonSchemaBuilder()
				.Type(SchemaValueType.Integer)
				.OptionalData(("minimum", "http://json.test/data#/minValue"))
			)
		);
	...
	{
		var instanceData = "{\"minValue\":true,\"foo\":{\"bar\":10}}";
		var instance = JsonDocument.Parse(instanceData).RootElement;
	...
	{
		var instanceData = "{\"minValu\":5,\"foo\":{\"bar\":10}}";
		var instance = JsonDocument.Parse(instanceData).RootElement;
